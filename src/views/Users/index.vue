...
import SearchAdmin from '@/components/Tables/Searchable/SearchAdmin.vue'
...
const loadAdmins = async (page = null) => {
  loading.value = true
  
  http.value.action('admins').withToken().paginate(page)
      .then(res => admins.value = res.data).catch(err => console.error(err))
...
const submitDelete = async () => {
  loading.value = true;
  try {
    await HTTP.action(`admins/${userToDelete.value}`).withToken().delete();
    await loadAdmins();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Admins'"/>
    <div class="flex flex-col gap-10">
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="flex flex-col md:flex-row justify-between mb-4">
          <SearchAdmin @search="searchAction" />
        </div>
        <div class="flex flex-col md:flex-row justify-between mb-4">
  <Search placeholder="Search" @search="searchActionFast"/>
  <primary-button @click="AddUser" class="mt-4 md:mt-0 md:ml-4">
    Add Admin
  </primary-button>
</div>
          ...
          <Datatable>
            <template #head>
              <tr class="uppercase dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3">
                  Roles
                  <!-- <SortableCoulmn filter="roles" name="role" @order-by="orderBy"/> -->
                </th>
                <th scope="col" class="py-3">
                  Status
            ...
            <template #body>
              <tr class="border-b border-[#f9fbfc]" v-for="(item, index) in admins.data" :key="index">
                <th class="px-3">
                  {{ ++index }}
                </th>
                <td class="py-2">
                ...
                <td class="py-2 px-1">
                  {{ item?.details?.status===1 ? 'Active' :'Inavtive'}}

                </td>
                <td class="py-2 text-center">

                  <button @click="EditUser(item.id)" class="hover:text-primary mr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="admins.links" @prev="args => loadAdmins(args)" @next="args => loadAdmins(args)" />
            </template>
          </Datatable>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        <h2 class="text-xl font-semibold mb-4">Are you sure you want to delete this admin?</h2>
