...
const loadPackages = async (page = null) => {
  ...
  http.value.action('packages').withToken().paginate(page)
    .then(res => packages.value = res.data).catch(err => console.error(err))
...
const AddPackage = () => {
  router.push({name: 'packages.create'})
};
const EditPackage = (id) => {
  router.push({ name: 'packages.edit', params: { id } });
};
...
const checkPackageRelations = async (id) => {
  try {
    const res = await HTTP.action(`packages/${id}`).withToken().get();
    const packageRelations = res.data.data;
    if (packageRelations.is_in_offer ) {
...
const remove = (id) => {
  packageToDelete.value = id;
  checkPackageRelations(id);
};

const submitDelete = async () => {
  loading.value = true;
  try {
    await HTTP.action(`packages/${packageToDelete.value}`).withToken().delete();
    await loadPackages();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Packages'" />
      ...
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="max-w-full overflow-x-auto">
          <div class="flex flex-col md:flex-row justify-between mb-4">
            ...
            <primary-button @click="AddPackage" class="mt-4 md:mt-0 md:ml-4">
              Add Package
            </primary-button>
          </div>
          <Datatable>
            ...
            <template #body>
              <tr class="border-b border-bodydark" v-for="(item, index) in packages.data" :key="index">
                <th class="px-3">
                  {{ ++index }}
                </th>
                ...
                <td class="px-3 py-1 text-center">
                  ...
                  <button  @click="EditPackage(item.id)" class="hover:text-primary">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="packages.links" @next="args => loadPackages(args)" @prev="args => loadPackages(args)"/>
            </template>
          </Datatable>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-50 dark:bg-[#1a222c]">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3  dark:bg-[#1a222c]">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Are you sure you want to delete this package?</h2>
    ...
    <div v-if="showWarningModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        ...
        <p class="text-xl font-semibold mb-4">
          This package has associated with offer. You can't delete it .
        </p>
