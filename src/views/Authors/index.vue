...
const loadauthors = async (page = null) => {
  try {
    loading.value = true;
    const res = await http.value.action('authors').withToken().paginate(page);
...
const EditAuthor = (id) => {
  router.push({ name: 'authors.edit', params: { id } });
};

const remove = (id) => {
  authorToDelete.value = id;
  checkAuthoredBooks(id);
};

const checkAuthoredBooks = async (id) => {
  try {
    const res = await HTTP.action(`authors/${id}`).withToken().get();
    const authoredBooks = res.data.author.has_books;
    if (authoredBooks) {
...
const confirmDelete = async () => {
  ...
  try {
    await HTTP.action(`authors/${authorToDelete.value}`).withToken().delete();
    await loadauthors();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Authors'" />
      ...
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="flex flex-col md:flex-row justify-between mb-4">
          ...
          <div class="flex flex-col md:flex-row justify-between mb-4">
            <Search placeholder="Search " @search="searchActionFast"/>
              <primary-button @click="AddAuthor" class="mt-4 md:mt-0 md:ml-4">
                Add Author
              </primary-button>
          </div>
        <div class="max-w-full overflow-x-auto">
          ...
          <Datatable>
            ...
            <template #body>
              <tr class="border-b border-bodydark" v-for="(item, index) in authors.data" :key="index">
                <td class="px-3 py-1">
                  {{ ++index }}
                </td>
                <td class="py-1">
                ...
                <td class=" py-1 text-center">
                  ...
                  <button  @click="EditAuthor(item.id)" class="hover:text-primary pr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="authors.links" @next="args => loadauthors(args)" @prev="args => loadauthors(args)"/>
            </template>
          </Datatable>
        </div>
        <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        <h2 class="text-xl font-semibold mb-4">Are you sure you want to delete this author?</h2>
    ...
    <div v-if="showWarningModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        ...
        <p class="text-xl font-semibold mb-4">
          This author has books associated with them. You can't delete it .
        </p>
