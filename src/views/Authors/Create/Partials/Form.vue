...
watch(() => form.value.country, () => {
  form.value.phone = form.value.country ? form.value.country.phone_code : '';
});
...
const submit = async () => {
  ...
  formData.append('password_confirmation', '12341234');
  ...
  await HTTP.action('authors').withToken().post(formData).then(res => {
      ...
      <template #form>
        ...
        <div class="col-span-6">
          ...
          <TextInput
            ...
            type="text"
            placeholder="Type author's nickname"
            class="mt-1 block w-full"
          />
