...
const loadCurrencies = async (page = null) => {
  loading.value = true
  http.value.action('currencies').withToken().paginate(page)
    .then(res => currencies.value = res.data).catch(err => console.error(err))
...
const submitDelete = async () => {
  loading.value = true;
  try {
    await HTTP.action(`currencies/${currencyToDelete.value}`).withToken().delete();
    await loadCurrencies();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Currencies'"/>
        ...
        <div class="max-w-full overflow-x-auto">
          <div class="flex flex-col md:flex-row justify-between mb-4">
            <Search placeholder="Search " @search="searchAction"/>
              <primary-button @click="AddCurrency" class="mt-4 md:mt-0 md:ml-4">
                Add Currency
              </primary-button>
          ...
          <Datatable>
            ...
            <template #body>
              <tr class="border-b border-bodydark" v-for="(item, index) in currencies.data" :key="index">
                <td class="px-3 py-1">
                  {{ ++index }}
                </td>
                <td class="py-2">
                ...
                <td class="py-2">
                  <span v-if="item.symbol_left">{{ item.symbol_left }}</span>
                  {{ parseFloat(item.value).toFixed(item.decimal_place) }}
                  <span v-if="item.symbol_right">{{ item.symbol_right }}</span>
                </td>
                <td class="py-2 text-center">
                  <button @click="EditCurrency(item.id)" class="hover:text-primary mr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="currencies.links" @next="args => currencies(args)" @prev="args => currencies(args)"/>
            </template>
          </Datatable>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-50 dark:bg-[#1a222c]">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3  dark:bg-[#1a222c]">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Are you sure you want to delete this Currency?</h2>
