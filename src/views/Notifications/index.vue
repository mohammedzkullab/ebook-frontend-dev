...
import SearchNotification from '@/components/Tables/Searchable/SearchNotification.vue'
...
const loadNotifications = async (page = null) => {
  loading.value = true;
  http.value.action('notification-details').withToken().paginate(page)
    .then(res => {
...
const submitDelete = async () => {
  loading.value = true;
  console.log(notificationToDelete.value)
  try {
    await HTTP.action(`notification-details/${notificationToDelete.value}`).withToken().delete();
    await loadNotifications();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
const send = (id) => {
  const confirmSend = confirm("Are you sure you want to send this notification?");
    if (confirmSend) {
    loading.value = true
    HTTP.action(`notification-details/${id}/send`).withToken().post().then(async () => {
      await loadNotifications()
    }).finally(() => loading.value = false)
  }
}
...
const EditNotifications = (id) => {
  router.push({ name: 'Notifications.edit', params: { id } });
};
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Notifications'" />
    <div class="flex flex-col gap-10">
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="flex flex-col md:flex-row justify-between mb-4">
          <SearchNotification @search="searchAction" />
        </div>
        <div class="flex flex-col md:flex-row justify-between mb-4">
          
            <Search placeholder="Search" @search="searchActionFast"/>
       
            <PrimaryButton @click="AddNotification" class="mt-4 md:mt-0 md:ml-4">Add Notification</PrimaryButton>
        </div>
        <div class="max-w-full overflow-x-auto">
          <Datatable>
            <template #head>
              <tr class="uppercase dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="created_at" name="Send Date" @order-by="orderBy"/>
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="type" name="Type" @order-by="orderBy"/>
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="status" name="Status" @order-by="orderBy"/>
                </th>
            ...
            <template #body>
              <tr class="border-b border-bodeydark" v-for="(item, index) in notifications.data" :key="index">
                <td class="py-1 px-2">{{ ++index }}</td>
                <td class="py-1">{{ item.title }}</td>
                <td class="py-1">{{ item.message }}</td>
                <td class="py-1">{{item.created_at}}</td>
                <td class="py-1">{{ item.type }}</td>
                <td class="py-1">
                  <span v-if="item.status === 1">Not Sent</span>
                  <span v-else-if="item.status === 2">Scheduled</span>
                  <span v-else-if="item.status === 3">Sent</span>
                </td>
                <td class="px-3 py-1 text-center">
                  <button  @click="EditNotifications(item.id)" class="hover:text-primary pr-2">
                    <svg
                    v-if="item.status === 1"
                      class="fill-current"
                      width="18"
                  ...
                  <button @click="remove(item.id)" class="hover:text-primary pr-2">
                    <svg 
                    v-if="item.status === 1"
                    class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <button @click="send(item.id)" class="hover:text-primary">
                    <svg 
                    v-if="item.status === 1"
                    class="fill-current" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M2.01 21L23 12L2.01 3L2 10L17 12L2 14L2.01 21Z" fill="currentColor"></path>
                    </svg>
                  </button>
                </td>
              </tr>
            ...
            <template #pagination>
              <Pagination :data="notifications.links" @next="args => loadNotifications(args)" @prev="args => loadNotifications(args)"/>
            </template>
          </Datatable>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-50 dark:bg-[#1a222c]">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3  dark:bg-[#1a222c]">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Are you sure you want to delete this notification?</h2>
