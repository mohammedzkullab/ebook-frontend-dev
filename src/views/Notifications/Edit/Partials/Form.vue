...
const loadNotificationData = async () => {
  loading.value = true;
  try {
    const res = await HTTP.action(`notification-details/${notificationId}`).withToken().get();
    const data = res.data.data;
    const [date, time] = data.send_date.split(' ');
    form.value = {
      title: data.title,
      image: null,
...
const submit = async () => {
  ...
  await HTTP.action(`notification-details/${notificationId}`).withToken().post(formData, {
    headers: {
...
<template>
  <FormSection @submitted="submit">
    ...
    <template #description>Edit the notification details.</template>
    <template #form>
      ...
      <div class="col-span-6">
        <InputLabel for="text" value="Message" />
        <TextArea id="message" v-model="form.message" type="text" placeholder="Enter Notification Text" class="mt-1 block w-full" />
