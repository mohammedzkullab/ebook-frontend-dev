...
const loadRoles = async (page = null) => {
  loading.value = true
  HTTP.action('roles').withToken().paginate(page)
      .then(res => roles.value = res.data.data.filter(role =>['ADMIN','SUPER_ADMIN'].includes(role.name))).catch(err => console.error(err))
...
const EditRole = (name) => {
  router.push({ name: 'roles.edit', params: { name } });
};
...
const groupPermissions = (permissions) => {
  ...
  permissions.forEach(permission => {
    const entity = permission.name.split('-').slice(1).join('-');
    if (!grouped[entity]) {
      grouped[entity] = [];
    }
...
const submitDelete = async () => {
  loading.value = true;
  try {
    await HTTP.action(`roles/${roleToDelete.value}`).withToken().delete();
    await loadRoles();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
          ...
          <Datatable>
            <template #head>
              <tr class="uppercase dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3 px-3" style="width: 5%;">
                  NO
                </th>
...
<!--                </th>-->
                ...
                <th scope="col" class="py-3 px-6" style="justify-items: center">
            ...
            <template #body>
              <tr class="border-b border-[#f9fbfc]" v-for="(item, index) in roles" :key="index">
...
<!--                <td class="py-2">-->
<!--                  <div v-for="(group, groupIndex) in groupPermissions(item.permissions)" :key="groupIndex" class="mb-1">-->
<!--                  <span v-for="permission in group" :key="permission.name" class="inline-block px-2">-->
<!--                    {{ permission.name }} <span v-if="groupIndex < item.permissions.length - 1"> - </span>-->
<!--                  </span>-->
<!--                  </div>-->
<!--                </td>-->

              <!-- <td class="py-2">
                  <div v-for="(permission, pIndex) in item.permissions" :key="pIndex">
               {{ permission.name }}
                </div>
                </td> -->
                ...
                <td class="py-2 text-center">
                  ...
                  <button @click="EditRole(item.name)" class="hover:text-primary mr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="roles.links" @prev="args => loadRoles(args)" @next="args => loadRoles(args)" />
            </template>
          </Datatable>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        <h2 class="text-xl font-semibold mb-4">Are you sure you want to delete this role?</h2>
