...
const loadCarts = async (page = null) => {
  loading.value = true
  http.value.action('carts').withToken().paginate(page)
    .then(res => carts.value = res.data.data).catch(err => console.error(err))
...
const view = (id) => {
  router.push({ name: 'carts.view', params: { id: id } })
}
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Carts'" />

    <div class="flex flex-col md:flex-row justify-between">
      <Search placeholder="Search" @search="searchAction"/>
      <div class="flex gap-2"></div>
      <div class="text-xl font-bold">Pending Carts: {{ cartCount }}</div>
    </div>
          ...
          <Datatable>
            <template #head>
              <tr class="uppercase dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3">Creation Date</th>
                <th scope="col" class="py-3">Book Count</th>
                <th scope="col" class="py-3">Total</th>
                <th scope="col" class="py-3">Coupon</th>
            ...
            <template #body>
              <tr class="border-b border-[#f9fbfc]" v-for="(item, index) in carts" :key="index">
                <td class="px-3 py-1">
                  {{ ++index }}
                </td>
                <td class="py-1">
                ...
                <td class="py-1">{{ TimeAdapter.convert(item.created_at) }}</td>
                <td class="py-1">{{ item.items_count }}</td>
                <td class="py-1">{{ item.total_price }}</td>
                <td class="py-1 underline text-blue-600">
                  <RouterLink :to="{name: 'codes.index'}">
                    {{ item.coupon?.name ?? '' }}
                  </RouterLink>
                </td>
                <td class="py-2">
            ...
            <template #pagination>
              <Pagination :data="carts.links" @next="args => loadCarts(args)" @prev="args => loadCarts(args)"/>
            </template>
          </Datatable>
