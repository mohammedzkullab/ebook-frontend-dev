...
const loadCategory = async () => {
  loading.value = true;
  try {
    const res = await HTTP.action(`categories/${categoryId}`).withToken().get()
    const data = res.data.data
    form.value = {
      name: data.name,
      type: data.type,
      parentClassification: data.parent_id,
      status:Boolean(data.status),
      image: null
    }
    photoPreview.value = data.image?.original_url || null;
...
const submit = async () => {
  ...
  await HTTP.action(`categories/${categoryId}`).withToken().post(formData, {
    headers: {
...
const onTypeChange = () => {
  if (form.value.type !== 'sub') {
    form.value.parentClassification = ''
  }
}
...
<template>
  ...
  <FormSection @submitted="submit">
    ...
    <template #description> Update the Category details.</template>
    ...
    <template #form>
      ...
      <div class="col-span-6">
        ...
        <select
            v-model="form.parentClassification"
            id="parent_id"
