...
const loadOffers = async (page = null) => {
  loading.value = true
  http.value
    .action('offers')
    .withToken()
...
const EditOffer = (id) => {
  router.push({ name: 'offers.edit', params: { id } })
}
...
const submitDelete = async () => {
  loading.value = true
  try {
    await HTTP.action(`offers/${offerToDelete.value}`).withToken().delete()
    await loadOffers()
    closeDeleteModal()
  } catch (error) {
    if (error.response && error.response.data && error.response.data.message) {
      ...
      deleteErrorMessage.value = 'An error occurred while trying to delete the offer.'
    }
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Offers'" />
    <div class="flex flex-col gap-10">
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="flex flex-col md:flex-row justify-between mb-4">
          <SearchOffer @search="searchAction" />
        </div>
        <div class="flex flex-col md:flex-row justify-between mb-4">
            <Search placeholder="Search" @search="searchActionFast" />
            <primary-button @click="AddOffer" class="mt-4 md:mt-0 md:ml-4">Add Offer</primary-button>
        </div>
          ...
          <Datatable>
            <template #head>
              <tr class="uppercase dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3">Type Name</th>
                <th scope="col" class="py-3">Discount</th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="start_date" name="Start Date" @order-by="orderBy" />
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="end_date" name="End Date" @order-by="orderBy" />
                </th>
            ...
            <template #body>
              <tr
                ...
                v-for="(item, index) in offers.data"
                :key="index"
              >
                <th class="px-3">
                  {{ ++index }}
              ...
              <strong>Package:</strong>
               ...
               <li>{{ item.package.name }}</li>
                ...
                <td class="py-1">
                  {{ item.discount_price ? item.discount_price : item.discount_percentage }}
                </td>
                <td class="py-1">
                  {{ item.start_date }}
                </td>
                ...
                <td class="px-3 py-1 text-center">
                  <button @click="EditOffer(item.id)" class="hover:text-primary pr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination
                :data="offers.links"
                @next="(args) => loadOffers(args)"
                @prev="(args) => loadOffers(args)"
              />
            </template>
          </Datatable>
        </div>
