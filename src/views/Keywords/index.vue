...
const loadKeywords = async (page = null) => {
  loading.value = true
  http.value.action('keywords').withToken().paginate(page)
    .then(res => keywords.value = res.data).catch(err => console.error(err))
...
const submitDelete = async () => {
  loading.value = true;
  try {
    await HTTP.action(`keywords/${keywordToDelete.value}`).withToken().delete();
    await loadKeywords();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
const AddKeyword = () => {
  router.push({name: 'keywords.create'})
};

const EditKeyword = (id) => {
  router.push({ name: 'keywords.edit', params: { id } });
};

</script>
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Keywords'"/>
        ...
        <div class="max-w-full overflow-x-auto">
          <div class="flex flex-col md:flex-row justify-between mb-4">
            <Search placeholder="Search " @search="searchAction"/>
              <primary-button @click="AddKeyword" class="mt-4 md:mt-0 md:ml-4">
                Add Keyword
              </primary-button>
          ...
          <Datatable>
            ...
            <template #body>
              <tr class="border-b border-bodydark" v-for="(item, index) in keywords.data" :key="index">
                <td class="px-3 py-1">
                  {{ ++index }}
                </td>
                <td class="py-2">
                ...
                <td class="py-2 text-center">
                  <button @click="EditKeyword(item.id)" class="hover:text-primary mr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="keywords.links" @next="args => loadKeywords(args)" @prev="args => loadKeywords(args)"/>
            </template>
          </Datatable>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-50 dark:bg-[#1a222c]">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3  dark:bg-[#1a222c]">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Are you sure you want to delete this Keyword?</h2>
