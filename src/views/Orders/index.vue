...
const loadOrders = async (page = null) => {
  loading.value = true
  http.value.action('orders').withToken().paginate(page)
    .then(res => orders.value = res.data).catch(err => console.error(err))
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Orders'" />
    <div class="flex flex-col gap-10">
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="max-w-full overflow-x-auto">
          <div class="flex flex-col md:flex-row justify-between mb-4">
            <Search placeholder="Search" @search="searchAction"/>
          ...
          <Datatable>
            <template #head>
              <tr class="uppercase  dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="created_at" name="Date" @order-by="orderBy"/>
                </th>
                <th scope="col" class="py-3">
                  Book No.
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="grand_total_price" name="Total Price" @order-by="orderBy"/>
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="order_status.slug" name="status" @order-by="orderBy"/>
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="payment_method.name" name="Payment Method" @order-by="orderBy"/>
                </th>
            ...
            <template #body>
              <tr class="border-b border-bodydark" v-for="(item, index) in orders.data" :key="index">
                <th class="px-3">
                  {{ ++index }}
                </th>
                <td class="py-1">
                ...
                <td class="py-1">
                  {{ item.grand_total_price }}
                </td>
                <td class="py-1">
                  {{ item.order_status.slug }}
                </td>
                <td class="py-1">
                ...
                <td class="px-3 py-1 text-center">
                  <button @click="Show(item.id)" class="p-2 rounded-full hover:bg-gray-200 hover:text-primary transition duration-200">
                    <svg class="fill-current" width="19" height="19" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path d="M12 4.5C7.5 4.5 3.73 7.61 2 12c1.73 4.39 5.5 7.5 10 7.5s8.27-3.11 10-7.5C20.27 7.61 16.5 4.5 12 4.5zm0 11.25A3.75 3.75 0 1 1 15.75 12 3.75 3.75 0 0 1 12 15.75zm0-6.25a2.5 2.5 0 1 0 2.5 2.5 2.5 2.5 0 0 0-2.5-2.5z"/>
                    </svg>
                  </button>
                  <button @click="Print()" class="hover:text-primary">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
                    ...
                    >
                      <path
                        d="M19 8h-1V3H6v5H5c-1.1 0-2 .9-2 2v7c0 1.1.9 2 2 2h3v4h8v-4h3c1.1 0 2-.9 2-2v-7c0-1.1-.9-2-2-2zm-4 11H9v-4h6v4zm2-14v3H7V5h10zm1 8h-4v-2h4v2z"
                        fill="currentColor"
                      />
                    </svg>
                  </button>
            ...
            <template #pagination>
              <Pagination :data="orders.links" @next="args => loadOrders(args)" @prev="args => loadOrders(args)"/>
            </template>
          </Datatable>
