...
import SearchUser from '@/components/Tables/Searchable/SearchUser.vue'
...
const loadUsers = async (page = null) => {
  loading.value = true
  http.value.action('users').withToken().paginate(page)
    .then(res => users.value = res.data).catch(err => console.error(err))
...
const view = (id) => {
  router.push({ name: 'readers.view', params: { id: id } })
};

const changeUserStatus = async (userId, newStatus, currentStatus) => {
  try {
    const statusToSend = newStatus === 0 && currentStatus === undefined ? true : newStatus;
    await HTTP.action(`users/${userId}`).withToken().put({
      status: statusToSend,
    });
    await loadUsers();
  } catch (error) {
    ...
    <div class="flex flex-col gap-10">
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="flex flex-col md:flex-row justify-between mb-4">
          <SearchUser @search="searchAction" />
        </div>
          ...
          <Datatable>
            <template #head>
              <tr class="uppercase dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="order" name="Orders" @order-by="orderBy" />
                </th>

                <th scope="col" class="py-3">
                  <SortableCoulmn filter="Total" name="Total Purchase" @order-by="orderBy" />
                </th>
                <th scope="col" class="py-3">
            ...
            <template #body>
              <tr class="border-b border-[#f9fbfc]" v-for="(item, index) in users.data" :key="index">
                <th class="px-3">
                  {{ ++index }}
                </th>
                <th class="py-2">
                ...
                <td class="py-2 flex items-center gap-2">
                  <span
                      :class="[
                        ...
                        item?.details?.status === 1 ? 'bg-green-500' : 'bg-red',
                      ]"
                    >
                  {{ item?.details?.status === 1 ? 'Active' : 'Inactive' }}
                </span>
                  <SwitchTwo
                    :id="'status-' + item.id"
                    :model-value="item?.details?.status ?? false"
                    @update:model-value="(newValue) => changeUserStatus(item.id, newValue, item?.details?.status)"
                  />
                </td>
            ...
            <template #pagination>
              <Pagination :data="users.links" @prev="args => loadUsers(args)" @next="args => loadUsers(args)" />
            </template>
          </Datatable>
        </div>
      </div>
