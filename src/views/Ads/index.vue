...
import SearchAd from '@/components/Tables/Searchable/SearchAd.vue'
...
const loadAds = async (page = null) => {
  loading.value = true
  http.value.action('ads').withToken().paginate(page)
    .then(res => ads.value = res.data).catch(err => console.error(err))
...
const submitDelete = async () => {
  loading.value = true;
  try {
    await HTTP.action(`ads/${adToDelete.value}`).withToken().delete();
    await loadAds();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
const AddAd = () => {
  router.push({name: 'Ads.create'})
};

const EditAd = (id) => {
  router.push({ name: 'Ads.edit', params: { id } });
};

</script>
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Ads'"/>
    <div class="flex flex-col gap-10">
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="flex flex-col md:flex-row justify-between mb-4">
          <SearchAd @search="searchAction" />
        </div>
        <div class="flex flex-col md:flex-row justify-between mb-4">
            <Search placeholder="Search" @search="searchActionFast"/>
            <primary-button @click="AddAd" class="mt-4 md:mt-0 md:ml-4">
              Add Ad
            </primary-button>  
        </div>
        <div class="max-w-full overflow-x-auto">
          <Datatable>
            ...
            <template #body>
              <tr class="border-b border-bodydark" v-for="(item, index) in ads.data" :key="index">
                <td class="px-3 py-1">
                  {{ ++index }}
                </td>
                <td class="py-1">
                ...
                <td class="py-2 text-center">
                  <button @click="EditAd(item.id)" class="hover:text-primary mr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="ads.links" @next="args => loadAds(args)" @prev="args => loadAds(args)"/>
            </template>
          </Datatable>
        </div>
      </div>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-50 dark:bg-[#1a222c]">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3  dark:bg-[#1a222c]">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Are you sure you want to delete this ad?</h2>
