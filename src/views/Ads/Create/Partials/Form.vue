...
const photoPreviewWeb = ref(null);
...
const submit = async () => {
  ...
  await HTTP.action('ads').withToken().post(formData).then(res => {
...
<template>
  ...
  <FormSection @submitted="submit">
    ...
    <template #description>Add a new Ad.</template>
    <template #form>

      <div class="col-span-6">
        <InputLabel for="title" value="Title" />
        <TextInput id="title" v-model="form.title" type="text" placeholder="Enter Title" class="mt-1 block w-full" />
        <span v-if="errors.title" class="text-red font-bold text-xs">{{ errors.title?.join() }}</span>
      </div>

      <div class="col-span-6">
        <InputLabel for="adType" value="Ad Type" />
      ...
      <div class="col-span-6">
        <InputLabel for="imageWeb" value="Website Image" />
        <label
          for="imageWeb"
          class="z-0 flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 relative overflow-hidden"
        >
      ...
      <div class="col-span-6 mt-4 ml-auto just">
