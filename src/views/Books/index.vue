...
import SearchBook from '@/components/Tables/Searchable/SearchBook.vue'
...
const loadBooks = async (page = null) => {
  ...
  http.value.action('books').withToken().paginate(page)
      .then(res => books.value = res.data).catch(err => console.error(err))
...
const checkBookRelations = async (id) => {
  try {
    const res = await HTTP.action(`books/${id}`).withToken().get();
    const bookRelations = res.data.data;
    if (bookRelations.is_in_cart || bookRelations.is_in_library || bookRelations.is_in_order
      ||  bookRelations.is_in_package ||  bookRelations.is_in_offer 
    ) {
...
const remove = (id) => {
  ...
  checkBookRelations(id);
};

const submitDelete = async () => {
  loading.value = true;
  try {
    await HTTP.action(`books/${bookToDelete.value}`).withToken().delete();
    await loadBooks();
    closeDeleteModal();
  } catch (error) {
    console.error(error);
...
const EditBook = (id) => {
  router.push({ name: 'books.edit', params: { id } });
};
...
<template>
  <DefaultLayout :loading="loading">
    <BreadcrumbDefault :pageTitle="'Books'" />
    <div class="flex flex-col gap-10">
      <div class="pt-6 pb-2.5 dark:border-strokedark xl:pb-1">
        <div class="flex flex-col md:flex-row justify-between mb-4">
            <SearchBook @search="searchAction" />
          </div>
        <div class="flex flex-col md:flex-row justify-between mb-4">
            <Search placeholder="Search " @search="searchActionFast"/>
            <primary-button @click="AddBook" class="mt-4 md:mt-0 md:ml-4">
              Add Book
            </primary-button> 
        </div>
          ...
          <Datatable>
            <template #head>
              <tr class="uppercase dark:bg-[#24303f]">
                ...
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="author.name" name="Author Name" @order-by="orderBy"/>
                </th>
                <th scope="col" class="py-3">
                  Category
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="house.name" name="publishing house" @order-by="orderBy"/>
                </th>
                <th scope="col" class="py-3">
                  Country
                </th>
                <th scope="col" class="py-3">
                  <SortableCoulmn filter="price" name="price" @order-by="orderBy"/>
                </th>
            ...
            <template #body>
              <tr class="border-b border-[#f9fbfc]" v-for="(item, index) in books?.data" :key="index">
                <td class="px-2 py-1">
                  {{ ++index }}
                </td>
                <td class="py-1" >
                  <img :src="item.main_image?.original_url ?? ''" alt="Book Image" class="w-14 h-auto rounded object-cover" />
                </td>
                <td class="py-1">
                  {{ item.name }}
                </td>
                ...
                <td class="py-1">
                  {{ item.price === null || item.price === 0 ? 'مجاني' : item.price }}
                </td>
                <td class="px-3 py-1 text-center">
                  <button  @click="EditBook(item.id)" class="hover:text-primary pr-2">
                    <svg
                      class="fill-current"
                      width="18"
                      height="18"
            ...
            <template #pagination>
              <Pagination :data="books.links" @next="args => loadBooks(args)" @prev="args => loadBooks(args)"/>
            </template>
          </Datatable>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 dark:bg-opacity-50 dark:bg-[#1a222c]">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3  dark:bg-[#1a222c]">
        <h2 class="text-xl font-semibold mb-4 dark:text-white">Are you sure you want to delete this book?</h2>
    ...
    <div v-if="showWarningModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        ...
        <p class="text-xl font-semibold mb-4">
          This Book has associated with others relations. You can't delete it .
        </p>
